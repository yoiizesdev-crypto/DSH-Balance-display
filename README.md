...
- **用量统计**（比官网多：不限 30 天，含安装前的历史）：
  ...
  - 数据来源：订阅 Harness 自身每次真实 LLM 调用（`session/event` 的 usage 字段）+ 历史会话日志回填，按 DeepSeek 官方峰谷定价（元/百万 tokens，9:00–12:00 / 14:00–18:00 高峰）估算费用，持久化在本地
- **关于与更新**：显示当前版本（v1.0.1），「自动检查更新」开关（默认开，启动后及每 6 小时自动检查）+「检查更新」按钮手动检测；发现新版本时提示并给出仓库链接。版本来源为 GitHub 仓库的 `vX.Y.Z` tag（发布者 `git tag vX.Y.Z && git push --tags`）
- **参数**（修改立即保存、立即生效）：
