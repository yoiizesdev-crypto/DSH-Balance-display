...
//     every successful refresh, so the 5 s heartbeat stays visible; flashes
//     silver on first load, green when the balance rises, red when it falls
//     — only for meaningful changes (≥ ¥0.05); cent-level drift updates the
//     number silently.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Changes below this (in CNY) update the text silently — no red/green
    // flash and no digit roll. Chatting drains the balance by cents at a
    // time, so cent-level drift would otherwise alarm on every message.
    var MIN_FLASH_DELTA = 0.05
      ...
      slots.inject('sidebar.footer.action', function () {
        return slots.register(
          ...
          function (props) {
            ...
            var breatheState = react.useState(0)
            var breathe = breatheState[0]
            var setBreathe = breatheState[1]
            ...
            var applyBalance = react.useCallback(function (next) {
              setDisplay(function (d) {
                ...
                if (d.text !== '--') {
                  var delta = Math.abs(next - Number(d.text))
                  if (delta < MIN_FLASH_DELTA) {
                    // tiny drift: keep the number current, but no alarm
                    return { text: text, prevText: d.prevText, dir: d.dir, flash: d.flash, tick: d.tick }
                  }
                }
                var flash = 'silver'
                var dir = d.dir
            ...
            var fetchBalance = react.useCallback(function () {
              fetchBalanceJson().then(function (data) {
                if (data !== null && typeof data === 'object' && typeof data.balance === 'number') {
                  ...
                  setBreathe(function (b) { return b + 1 })
                }
              }).catch(function () {})
            }, [applyBalance])
            ...
            var doRefresh = function () {
              ...
              fetchBalanceJson().then(function (data) {
                ...
                // keep the one-shot rotation visible for at least its full
                // duration, so a fast response never snaps the icon back
                var wait = Math.max(0, 500 - (Date.now() - t0))
                setTimeout(function () { setRefreshing(false) }, wait)
              })
            }
            ...
            return react.createElement('div', {
              ...
              react.createElement('span', { className: 'q-num' },
                react.createElement('span', { key: 'b' + breathe, className: 'q-breathe' },
