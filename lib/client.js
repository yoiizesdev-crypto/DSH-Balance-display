  ...
  factory: (require) => {
    ...
    function refreshBalance() {
      ...
      fetch('/quota/balance', { cache: 'no-store' })
        ...
        .then(function (data) {
          if (data !== null && typeof data === 'object' && typeof data.balance === 'number') {
            update({ balance: data.balance, error: null, lastUpdated: Date.now(), breathe: QUOTA_STATE.breathe + 1 })
          } else {
    ...
    function Sparkline(props) {
      ...
      return react.createElement('div', { className: 'qs-chart' },
        react.createElement('svg', { width: '100%', viewBox: '0 0 ' + W + ' ' + H, preserveAspectRatio: 'none' },
          ...
          react.createElement('circle', { cx: lastX, cy: lastY, r: 3, fill: 'var(--dsw-alias-state-success-primary)', stroke: 'var(--dsw-alias-bg-layer-2)', strokeWidth: 1.5 }),
        ),
    ...
    function QuotaSettingsSection(props) {
      ...
      var points = snap.history.filter(function (p) { return p.t >= cutoff })
      var minB = Infinity
      var maxB = -Infinity
      for (var i = 0; i < points.length; i++) {
      ...
      return react.createElement('div', { className: 'qs-section' },
        react.createElement('div', { className: 'qs-head' },
          ...
          react.createElement('p', { className: 'qs-intro' }, '实时显示 DeepSeek 账户余额与历史趋势。调整下方参数会立即保存并生效。'),
        ),
        react.createElement('div', { className: 'qs-card' },
          react.createElement('div', { className: 'qs-live-main' },
            ...
            react.createElement('span', { className: 'qs-live-num' }, snap.balance === null ? '--' : String(snap.balance)),
            react.createElement('span', { className: 'qs-live-unit' }, '¥'),
            react.createElement('button', {
              type: 'button',
        ...
        react.createElement('div', { className: 'qs-card' },
          react.createElement('div', { className: 'qs-card-head' },
            react.createElement('span', { className: 'qs-card-title' }, '余额趋势'),
          ...
          hasPoints ? react.createElement('div', { style: { flexDirection: 'column', gap: 6, display: 'flex' } },
            react.createElement(Sparkline, { points: points }),
            react.createElement('div', { className: 'qs-chart-stats' },
              '当前 ¥' + (snap.balance === null ? '--' : String(snap.balance)),
              ' · 区间最低 ¥' + (minB === Infinity ? '--' : minB.toFixed(2)),
              ' · 区间最高 ¥' + (maxB === -Infinity ? '--' : maxB.toFixed(2)),
            ),
    ...
    function BadgeComponent(props) {
      ...
      // Roll the odometer digits whenever the store's balance changes.
      react.useEffect(function () {
        var b = snap.balance
        if (b === null) return
