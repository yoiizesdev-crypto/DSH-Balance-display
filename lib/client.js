...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PRICING = {
      // ---- DeepSeek (official dual-currency, peak/off-peak). FLASH prices
      // are the 2026-09-10 12:00 Beijing rates (hit ¥0.02 / in ¥1.0 / out
      // ¥4.0 idle, peak ×2); V4-Pro keeps its previous list. ----
      'deepseek-v4-flash': {
    ...
    // Host transport
    //
    // Web profile: relative '/quota/*' paths reach the host's web server.
    // Electron Desktop: the Desktop Host answers unknown `dsh-app://` paths
    // with the SPA HTML, so the host half publishes
    // `window.__QUOTA_BADGE_HOST__ = { origin, token, ports }` through the
    // index-injection hook and this module routes every call to that loopback
    // origin with the per-process token. When the origin is not ready at
    // injection time the advertised ports are probed through GET /quota/health.
    // -----------------------------------------------------------------------

    var HOST_INFO = (typeof window !== 'undefined' && window.__QUOTA_BADGE_HOST__ !== null
      && typeof window.__QUOTA_BADGE_HOST__ === 'object') ? window.__QUOTA_BADGE_HOST__ : null
    /** Loopback ports the host half tries, in the same order (lib/index.js). */
    var LOOPBACK_PORTS = [17871, 17872, 17873, 17874, 17875, 17876, 17877, 17878, 17879, 17880]
    var hostBase = null // resolved origin; '' means same-origin (Web profile)
    var hostProbe = null // in-flight port probe
    var transportError = null // last transport-level failure (diagnostics)

    /** Advertised token, or '' when the page never received the injection. */
    function hostToken() {
      return HOST_INFO !== null && typeof HOST_INFO.token === 'string' ? HOST_INFO.token : ''
    }

    function probeHostPorts() {
      var ports = HOST_INFO !== null && Array.isArray(HOST_INFO.ports) && HOST_INFO.ports.length > 0
        ? HOST_INFO.ports
        : LOOPBACK_PORTS
      var token = hostToken()
      var index = 0
      function next() {
        if (index >= ports.length) {
          transportError = 'loopback ports ' + ports[0] + '-' + ports[ports.length - 1] + ' did not answer'
          return ''
        }
        var base = 'http://127.0.0.1:' + ports[index++]
        // /quota/health is the one route that answers without a token.
        return fetch(base + '/quota/health', { cache: 'no-store' })
          ...
          .then(function (body) {
            if (body !== null && typeof body === 'object' && body.ok === true) {
              ...
              // Host found but the page never got the injection: data routes
              // need the token, so report that instead of failing silently.
              transportError = '宿主已找到（' + base + '）但页面缺少注入的访问令牌'
              return ''
            }
            return next()
          })
    ...
    function resolveHostBase() {
      if (hostBase !== null) return Promise.resolve(hostBase)
      if (HOST_INFO === null) {
        var plain = typeof location === 'undefined' || location.protocol === 'http:' || location.protocol === 'https:'
        if (plain) {
          // Web profile: the real web server answers relative paths.
          hostBase = ''
          return Promise.resolve('')
        }
        // Desktop scheme without an injection: look for a loopback carrier so
        // the failure has a reason attached (and log it in the diagnostics).
        if (hostProbe === null) {
          hostProbe = probeHostPorts().then(function (base) {
            hostBase = ''
            update({
              error: transportError !== null
                ? transportError
                : '未收到宿主注入信息，且本机没有响应 17871-17880 的桥接端口',
            })
            return hostBase
          })
        }
      ...
      if (typeof HOST_INFO.origin === 'string' && HOST_INFO.origin !== '') {
        hostBase = HOST_INFO.origin
        return Promise.resolve(hostBase)
      }
      if (hostProbe === null) hostProbe = probeHostPorts()
      return hostProbe
    }

    /** fetch() against the host, adding the loopback origin + token when needed. */
    function quotaFetch(path, init) {
      return resolveHostBase().then(function (base) {
        var url = base + path
        if (base !== '') {
          var token = hostToken()
          if (token !== '') url += (path.indexOf('?') === -1 ? '?' : '&') + 'token=' + encodeURIComponent(token)
        }
    ...
    function refreshBalance() {
      ...
      quotaFetch('/quota/balance', { cache: 'no-store' })
    ...
    function loadConfig() {
      return quotaFetch('/quota/config', { cache: 'no-store' })
        .then(function (r) { return r.json() })
        .then(function (data) {
        ...
        .catch(function (err) {
          // This used to swallow the failure ("host half unavailable → keep
          // defaults"), which left the badge blank with no hint at all — the
          // single hardest symptom to diagnose. Surface it instead; the badge
          // paints an error dot and the settings page shows the transport.
          transportError = String(err !== null && typeof err === 'object' && err.message !== undefined ? err.message : err)
          update({ error: '宿主不可达（' + transportError + '）' })
        })
    }

    function saveConfig(patch) {
      return quotaFetch('/quota/config', {
        method: 'POST',
    ...
    function loadUsage() {
      ...
      quotaFetch('/quota/usage', { cache: 'no-store' })
        .then(function (r) { return r.json() })
        .then(function (data) {
      ...
      quotaFetch('/quota/update/apply', {
        ...
        .then(function (data) {
          if (data !== null && typeof data === 'object' && data.ok === true) {
            ...
            // A package-manager install (npm/pnpm/desktop plugin manager) must
            // not be overwritten in place: the host reports the upgrade command
            // instead, so keep it for the copyable hint in the settings card.
            var command = data !== null && typeof data === 'object' && typeof data.command === 'string' ? data.command : null
            update({ updating: false, updateResult: { ok: false, message: msg, command: command } })
          }
        })
        .catch(function () {
    ...
    function UpdateCard(props) {
      ...
      // Text handed to a human/AI when an in-plugin update fails. A managed
      // install (npm/pnpm/desktop plugin manager) reports the exact upgrade
      // command, which is what the user actually needs there.
      var updateResultForHelp = snap.updateResult
      var helpText = updateResultForHelp !== null && typeof updateResultForHelp === 'object'
        && typeof updateResultForHelp.command === 'string' && updateResultForHelp.command !== ''
        ? updateResultForHelp.command
        : '更新余额插件 ' + 'https://github.com/' + REPO_URL
    ...
    /**
     * Diagnostics card. "Nothing appears" used to be undiagnosable from the UI:
     * the host half could be pending forever, or the browser half could fail to
     * reach it, with no visible difference. This card reports the carrier the
     * host picked, whether this page can reach it, and the versions involved.
     */
    function DiagnosticsCard(props) {
      ...
      var loadHealth = function () {
        quotaFetch('/quota/health', { cache: 'no-store' })
          .then(function (r) { return r.json() })
          .then(function (data) { setHealth(data !== null && typeof data === 'object' ? data : { ok: false, error: 'empty health response' }) })
          .catch(function (err) {
            setHealth({ ok: false, error: String(err !== null && typeof err === 'object' && err.message !== undefined ? err.message : err) })
          })
      }
      react.useEffect(function () { loadHealth() }, [])
      var reachable = health !== null && health.ok === true
      var transport = health !== null && typeof health.transport === 'string'
        ? health.transport
        : (HOST_INFO === null ? 'webServer (same origin)' : 'unresolved')
      var rows = [
        { label: '宿主可达', value: reachable ? '是' : '否' },
        { label: '传输方式', value: transport + (health !== null && health.port !== null && health.port !== undefined ? ' · 127.0.0.1:' + String(health.port) : '') },
        { label: '插件版本', value: health !== null && typeof health.version === 'string' ? 'v' + health.version : '未知' },
        { label: '宿主 Node', value: health !== null && typeof health.node === 'string' ? health.node : '未知' },
        { label: '会话日志回填 (zstd)', value: health !== null && typeof health.zstdBackfill === 'boolean' ? (health.zstdBackfill ? '可用' : '不可用（Node < 22.15）') : '未知' },
        { label: '宿主请求数', value: health !== null && typeof health.requests === 'number' ? String(health.requests) : '—' },
        { label: '最近读取错误', value: typeof snap.error === 'string' && snap.error !== '' ? snap.error : '无' },
        { label: '宿主最近错误', value: health !== null && typeof health.lastError === 'string' && health.lastError !== '' ? health.lastError : '无' },
        { label: '页面传输错误', value: transportError !== null ? transportError : '无' },
      ]
      var report = rows.map(function (row) { return row.label + '：' + row.value }).join('\n')
        + '\n插件仓库：https://github.com/' + REPO_URL
        + '\n注入信息：' + (HOST_INFO === null ? '无（同源）' : JSON.stringify({ origin: HOST_INFO.origin, ports: HOST_INFO.ports }))
      var copyReport = function () {
        ...
        if (navigator.clipboard !== undefined && typeof navigator.clipboard.writeText === 'function') {
          navigator.clipboard.writeText(report).then(done).catch(function () { done() })
        } else {
          done()
        }
      }
      return react.createElement('div', { className: 'qs-card' },
        react.createElement('div', { className: 'qs-card-head' },
          react.createElement('span', { className: 'qs-card-title' }, '诊断'),
          react.createElement('button', {
            type: 'button',
            className: 'qs-update-help-copy' + (copied ? ' qs-copied' : ''),
            onClick: copyReport,
        ...
        rows.map(function (row) {
          return react.createElement('div', { className: 'qs-row', key: row.label },
            react.createElement('span', { className: 'qs-row-label' },
              react.createElement('span', { className: 'qs-row-title' }, row.label),
              react.createElement('span', { className: 'qs-row-hint' }, row.value),
            ),
          )
        }),
      )
    ...
    function QuotaSettingsSection(props) {
      ...
      return react.createElement('div', { className: 'qs-section' },
        ...
        react.createElement(DiagnosticsCard, { snap: snap }),
      )
    }
    ...
    // -----------------------------------------------------------------------
    // Price-panel preferences: remember the user's model checkbox selections
    // (and the currency axis) across panel opens — and across page reloads —
    // via localStorage. Falls back to defaults when storage is unavailable.
    // -----------------------------------------------------------------------

    var PANEL_PREF_KEY = 'dsh-quota-badge.panel'

    /** Read the stored { enabled: [modelKeys], currency } or null. */
    ...
    /** Persist { enabled, currency }; never throws. */
    ...
    function ChartPanel(props) {
      ...
      // Model selection state: DeepSeek models on by default, the rest
      // opt-in — restored from the persisted panel preferences if present.
      var enabledState = react.useState(function () {
        var prefs = loadPanelPrefs()
        var stored = prefs !== null && Array.isArray(prefs.enabled) ? prefs.enabled : null
        var set = {}
        if (stored !== null) {
          for (var i = 0; i < stored.length; i++) {
            var k = stored[i]
            if (PRICING[k] !== undefined) set[k] = true
          }
        }
        if (Object.keys(set).length === 0) {
          // First use or a stale store: default to the DeepSeek family.
          for (var j = 0; j < modelKeys.length; j++) {
            if (PRICING[modelKeys[j]].vendor === 'ds') set[modelKeys[j]] = true
          }
        }
        return set
      })
      ...
      // also restored from the persisted preferences.
      var currencyState = react.useState(function () {
        var prefs = loadPanelPrefs()
        return prefs !== null && prefs.currency === 'usd' ? 'usd' : 'cny'
      })
      var currency = currencyState[0]
      var setCurrency = currencyState[1]
      var currencyUnit = currency === 'usd' ? '$' : '¥'
      var persistPanel = function (nextEnabled, nextCurrency) {
        var keys = []
        for (var k2 in nextEnabled) {
          if (nextEnabled[k2] === true) keys.push(k2)
        }
        savePanelPrefs({ enabled: keys, currency: nextCurrency !== undefined ? nextCurrency : currency })
      }
        ...
        react.createElement('div', { className: 'q-pop-head' },
          ...
          react.createElement('div', { className: 'q-pop-head-actions' },
            react.createElement('button', {
              ...
              onClick: function () {
                var nextCurrency = currency === 'usd' ? 'cny' : 'usd'
                setCurrency(nextCurrency)
                persistPanel(enabled, nextCurrency)
              },
    ...
    function BadgeComponent(props) {
      ...
      // Collapsed rail: keep the dot + number, drop the label/phase/spacer.
      var compact = props.compact === true
      var hasError = typeof snap.error === 'string' && snap.error !== ''
      var effectiveDotStyle = hasError
        ? Object.assign({}, dotStyle, { color: 'var(--dsw-alias-state-error-primary, #ff5c5c)' })
        : dotStyle
      var status = hasError
        ? '余额读取失败：' + snap.error
        : (shown.value === null ? '余额加载中…' : '余额 ' + shown.value + ' ' + shown.symbol)
      return react.createElement('div', {
        ref: wrapRef,
        className: 'q-wrap',
        title: status,
        ...
        compact ? null : react.createElement('span', { className: 'q-label' },
          '余额',
    ...
    function apply(ctx) {
      // Boot the shared store FIRST: load persisted parameters, fetch once, and
      // prefetch the usage ledger and the cached update-check result. This must
      // not sit behind the `slots` lookup below — the previous
      // `var slots = ctx.get('slots'); if (slots === undefined) return` tied the
      // whole plugin (data included) to client load order. Client plugin
      // bundles are created concurrently by the shell, so `slots` is frequently
      // undefined at apply time and the plugin silently did nothing at all.
      loadConfig()
      refreshBalance()
      loadUsage()
      loadUpdate(false)
      ...
      // Wait for the slots service instead of giving up on it: cordis keeps this
      // callback until the service appears, whatever the load order.
      ctx.inject(['slots'], function (scoped) {
        var slots = scoped.get('slots')
      ...
      slots.inject('sidebar.footer.action', function () {
        return slots.register(
          ...
          function (props) {
            // `wide === false` is the collapsed rail. Returning null there used
            // to hide the whole feature for narrow sidebars; render the compact
            // badge instead (dot + number, no label).
            return react.createElement(BadgeComponent, { compact: props.wide !== true })
          },
        )
      })
