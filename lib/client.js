...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Weekday/weekend rule change effective 2026-08-23 00:00 (Beijing):
    // weekdays keep peak windows, weekends charge the off-peak price all day.
    var PRICING = {
    ...
    function priceOf(model, d, field) {
      return isPeak(d) ? model.peak[field] : model.idle[field]
    }
     ...
     */
    /** Official CNY→USD rate, mirrors the host's USD_RATE. */
    var USD_RATE = 7.14
    /** USD figure for one ledger node; old records carry only `cost` (CNY). */
    function usdOf(node) {
      var c = node !== null && typeof node === 'object' && typeof node.cost === 'number' ? node.cost : 0
      var u = node !== null && typeof node === 'object' && typeof node.costUsd === 'number' ? node.costUsd : 0
      return u > 0 ? u : c / USD_RATE
    }
        ...
        usage === null ? react.createElement('div', { className: 'qs-chart-empty' }, '正在读取用量数据…') : react.createElement('div', { style: { flexDirection: 'column', gap: 10, display: 'flex' } },
          react.createElement('div', { className: 'qs-usage-totals' },
            react.createElement('div', { className: 'qs-usage-cell' },
              ...
              react.createElement('span', { className: 'qs-usage-cell-label' }, '$' + fmtMoney(totals.costUsd)),
            ),
            react.createElement('div', { className: 'qs-usage-cell' },
          ...
          dayPoints.length >= 2
            ? react.createElement('div', { style: { flexDirection: 'column', gap: 6, display: 'flex' } },
                ...
                react.createElement('div', { className: 'qs-chart-stats' },
                  '区间消费 ¥' + fmtMoney(totals.cost) + ' / $' + fmtMoney(totals.costUsd),
                  ' · 日均 ¥' + fmtMoney(dayPoints.length > 0 ? totals.cost / dayPoints.length : 0) + ' / $' + fmtMoney(dayPoints.length > 0 ? totals.costUsd / dayPoints.length : 0),
                ),
              )
            ...
            react.createElement('tbody', null,
              modelRows.map(function (m) {
                ...
                return react.createElement('tr', { key: m },
                  ...
                  react.createElement('td', null, '¥' + fmtMoney(mm.cost) + ' / $' + fmtMoney(mm.costUsd)),
                )
              }),
            ),
          ) : null,
          react.createElement('div', { className: 'qs-usage-note' },
            '按 DeepSeek 官方峰谷定价（元/百万 tokens）估算，USD 按官方固定汇率 ¥1≈$0.14 折算；数据来自本机每次真实调用，安装插件起持续记录，不限于 30 天。',
          ),
        ),
      )
    }
    ...
    function ChartPanel(props) {
      ...
      var stepPoints = function (model, field) {
        ...
        for (var h = 0; h <= 24; h += 0.25) {
          var v = priceOf(model, hourDate(h), field)
      ...
      return react.createElement('div', { className: 'q-pop', style: style, onMouseEnter: props.onEnter, onMouseLeave: props.onLeave },
        ...
        react.createElement('svg', { width: W, height: H, viewBox: '0 0 ' + W + ' ' + H },
          ...
          models.map(function (m) {
            return react.createElement('circle', { key: 'dot' + m.label, cx: lineX, cy: my(priceOf(m, now, 'out')).toFixed(1), r: 3.2, fill: m.color, stroke: '#0b0d11', strokeWidth: 1 })
          }),
          X_TICKS.map(function (h) {
        ...
        react.createElement('div', { className: 'q-pop-info' },
          ...
          'Flash 输出 ' + priceOf(PRICING['deepseek-v4-flash'], now, 'out') + ' 元',
          ' / Pro 输出 ' + priceOf(PRICING['deepseek-v4-pro'], now, 'out') + ' 元',
        ),
        react.createElement('div', { className: 'q-pop-note' },
          weekendIdle
            ? '周末（周六/周日）全天按低谷价计费 · 工作日高峰 9:00-12:00 / 14:00-18:00（北京时间）· 来源：DeepSeek 官方定价（2026-08-23 起）'
            : '高峰 9:00-12:00 / 14:00-18:00（北京时间），其余空闲半价 · 来源：DeepSeek 官方定价（2026-08-17 起）',
        ),
      )
    }
