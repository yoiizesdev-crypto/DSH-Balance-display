...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var QUOTA_STATE = {
      ...
      manualAt: 0, // ms of the last MANUAL refresh success (roll animation)
      revision: 0,
    }
    ...
    function refreshBalance(source) {
      var now = Date.now()
        ...
        .then(function (data) {
          if (data !== null && typeof data === 'object' && Array.isArray(data.balances) && data.balances.length > 0) {
            ...
            if (keys.length > 0) {
              ...
              if (manual) patch.manualAt = patch.lastUpdated
              update(patch)
              return
            }
            update({ error: '查询失败' })
        ...
        react.createElement('div', { className: 'qs-card' },
          react.createElement('div', { className: 'qs-live-main' },
            ...
            react.createElement('button', {
              ...
              onClick: function () { refreshBalance('manual') },
              'aria-label': '立即刷新余额',
            }, REFRESH_ICON),
          ),
    ...
    function BadgeComponent(props) {
      ...
      var state = react.useState({ text: '--', prevText: null, dir: 'up', flash: 'silver', tick: 0, silent: false })
      var display = state[0]
      ...
      // Roll the odometer digits ONLY on manual refresh; auto-refreshes swap
      // the number silently (keeping prevText so the next manual roll still
      // has a starting point). Direction colors still flash on any change.
      react.useEffect(function () {
        var b = shown.value
        if (b === null) return
        var manual = snap.manualAt !== 0 && snap.manualAt === snap.lastUpdated
        setDisplay(function (d) {
          ...
          if (!manual) {
            // auto refresh: no roll animation, keep prevText for later
            return { text: text, prevText: d.prevText, dir: dir, flash: flash, tick: d.tick + 1, silent: true }
          }
          return {
            text: text,
            prevText: d.text !== '--' ? d.text : d.prevText,
            dir: dir,
            flash: flash,
            tick: d.tick + 1,
            silent: false,
      ...
      // silent (auto) updates pass no prev chars, so digits swap without rolling
      var prevChars = display.silent || display.prevText === null ? [] : display.prevText.split('')
      var hourFrac = now.getHours() + now.getMinutes() / 60
      var peakNow = isPeak(hourFrac)
